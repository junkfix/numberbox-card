((LitElement) => {
  console.info("NUMBERBOX_CARD 1.4");
  ...
  class NumberBox extends LitElement {
    ...
    static get styles() {
      return css`
        ...
        .body {
          ...
          padding-bottom: calc(
            var(--st-spacing, var(--st-default-spacing)) * 2
          );
        }
        .main {
          display: flex;
    ...
    render() {
      if (!this.stateObj) {
        return html`<ha-card>Missing:'${this.config.entity}'</ha-card>`;
      }
      const vars = {};
          ...
          ${vars.icon
            ? html`<div class="grid">
                <div
                  ...
                  @click="${() => this.moreInfo("hass-more-info")}"
                >
                  <div>
                    <ha-icon
                      icon="${vars.icon}"
                      style="margin-right:20px;color:var(--paper-item-icon-color);"
                    ></ha-icon>
                    ${vars.name}
                  </div>
                </div>
                <div class="grid-content grid-right">${this.renderNum()}</div>
              </div>`
            : this.renderNum()}
        </ha-card>
      `;
    ...
    incVal(dhis) {
      dhis._hass.callService("input_number", "increment", {
        entity_id: dhis.stateObj.entity_id,
      });
    }

    decVal(dhis) {
      dhis._hass.callService("input_number", "decrement", {
        entity_id: dhis.stateObj.entity_id,
      });
    }
